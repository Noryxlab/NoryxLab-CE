/**
 * French catalogue — the source of truth for the key set.
 *
 * The previous UI carried 31 `data-i18n` attributes over ~9400 lines of
 * hardcoded French, so the FR/EN switch translated almost nothing and the
 * English build was unusable. Here the English catalogue is typed against
 * this object, which makes a missing translation a compile error rather than
 * a string that silently stays French in front of a customer.
 *
 * Accents are deliberate: the old markup had lost them in a large share of
 * its strings ("Televerser", "Aucun dataset selectionne", "Activite"), which
 * reads as unfinished in a French-language demo.
 */
export const fr = {
  common: {
    actions: 'Actions',
    add: 'Ajouter',
    cancel: 'Annuler',
    close: 'Fermer',
    confirm: 'Confirmer',
    copy: 'Copier',
    create: 'Créer',
    delete: 'Supprimer',
    download: 'Télécharger',
    edit: 'Modifier',
    error: 'Erreur',
    export: 'Exporter',
    filter: 'Filtrer',
    loading: 'Chargement…',
    name: 'Nom',
    description: 'Description',
    open: 'Ouvrir',
    optional: 'facultatif',
    owner: 'Propriétaire',
    refresh: 'Actualiser',
    retry: 'Réessayer',
    save: 'Enregistrer',
    search: 'Rechercher',
    settings: 'Paramètres',
    status: 'État',
    createdAt: 'Créé le',
    updatedAt: 'Mis à jour',
    startedAt: 'Démarré',
    duration: 'Durée',
    project: 'Projet',
    type: 'Type',
    size: 'Taille',
    role: 'Rôle',
    user: 'Utilisateur',
    organization: 'Organisation',
    none: 'Aucun',
    unknown: 'Inconnu',
    yes: 'Oui',
    no: 'Non',
    required: 'Ce champ est obligatoire.',
    accessDenied: 'Vous n’avez pas les droits nécessaires pour cette action.',
    unreachable: 'La plateforme est injoignable. Vérifiez votre connexion réseau puis réessayez.',
    loadError: 'Chargement impossible',
    viewLogs: 'Voir les journaux',
    logs: 'Journaux',
  },

  nav: {
    administrator: 'Admin',
    home: 'Accueil',
    projects: 'Projets',
    catalog: 'Catalogue',
    agents: 'Agents',
    production: 'Production',
    administration: 'Administration',
    projectSection: 'Projet',
    overview: 'Vue d’ensemble',
    workspaces: 'Workspaces',
    jobs: 'Jobs',
    apps: 'Applications',
    apis: 'API',
    dashboards: 'Dashboards',
    data: 'Données',
    environments: 'Environnements',
    members: 'Membres',
    settings: 'Paramètres',
    datasets: 'Datasets',
    datasources: 'Sources de données',
    ontologies: 'Ontologies',
    repositories: 'Dépôts Git',
    secrets: 'Secrets',
    network: 'Réseau et sécurité',
    rbac: 'Accès',
    activity: 'Activité',
    audit: 'Audit',
    backups: 'Sauvegardes',
    identity: 'Utilisateurs et organisations',
    dataGovernance: 'Gouvernance des données',
    validation: 'Validation',
    inventory: 'Logiciels et licences',
    storage: 'Stockage',
    backToProjects: 'Tous les projets',
    documentation: 'Documentation',
    apiReference: 'Référence API',
    platformStatus: 'État de la plateforme',
    toggleSidebar: 'Réduire le menu',
    theme: 'Thème',
    themeLight: 'Clair',
    themeDark: 'Sombre',
    themeSystem: 'Système',
    language: 'Langue',
    signIn: 'Se connecter',
    signOut: 'Se déconnecter',
    account: 'Mon compte',
    skipToContent: 'Aller au contenu principal',
  },

  storageCapacity: {
    title: 'Place restante pour les volumes',
    intro:
      'Ce que le cluster peut encore accepter. Une demande de volume est acceptée à vue et n’échoue qu’au moment de l’attachement : sans cet écran, la première alerte est un utilisateur qui ne peut pas travailler.',
    claimedOf: 'réservés sur {total} allouables',
    freeDisk: '{free} de disque réellement inutilisé',
    leftToAllocate: '{left} encore allouables',
    nodeDetail: '{claimed} réservés sur {allocatable} · {free} de disque libre',
    tight: 'Bientôt plein',
    full: 'Plein',
    unavailable:
      'La couche de stockage de cette installation ne sait pas répondre. La plateforme ne peut donc pas prévenir à l’avance — elle préfère le dire plutôt que d’afficher une jauge inventée.',
    explain:
      'Un volume est placé en fonction de ce qu’il réserve, pas du disque libre. C’est pourquoi les deux chiffres diffèrent, et pourquoi un cluster presque vide peut refuser un volume de plus.',
  },
  roleMatrix: {
    title: 'Rôles de l’installation',
    intro:
      'Les rôles que la plateforme applique, et ceux que vous écrivez. Une ligne livrée décrit ce que la plateforme fait déjà et ne se modifie pas : c’est sur vos propres rôles que vous décidez.',
    addRole: 'Ajouter un rôle',
    basedOn: 'Hérite de',
    baseViewer: 'Lecteur',
    baseEditor: 'Contributeur',
    baseAdmin: 'Administrateur',
    columnProject: 'Projet',
    columnDataset: 'Données',
    columnOntology: 'Ontologie',
    columnDatasource: 'Source',
    columnEnvironment: 'Environnement',
    columnWorkload: 'Calcul',
    columnGovernance: 'Gouvernance',
    namePlaceholder: 'Data steward',
    descriptionPlaceholder: 'Ce que ce rôle fait, en une phrase',
    shipped: 'Livré',
    shippedBase: 'Rôle de la plateforme',
    heldBy: 'porté par {count}',
    governanceLocked: 'Décidée par l’annuaire',
    saved: 'Rôles enregistrés',
    ceilingHint:
      'Un rôle maison hérite toujours d’un rôle de la plateforme : c’est ce que la personne obtient partout où la matrice n’a rien à dire, et c’est aussi son plafond tant que la matrice n’applique pas la ligne.',
  },
  agentGovernance: {
    title: 'Agents',
    intro:
      'Ce que les agents de cette installation ont le droit de faire, et ce qu’ils ont fait. L’activité est comptée sur les actions relevées par la plateforme, jamais sur ce que leurs rapports affirment.',
    agents: 'Agents',
    canAct: 'Peuvent agir',
    canActHint: 'Les autres regardent et rapportent',
    mandates: 'Demandes autorisées',
    actions: 'Actions effectuées',
    window: 'Sur {days} jours',
    mayDo: 'Droit d’agir',
    state: 'État',
    activity: 'Activité',
    runsAndActions: '{runs} passages · {actions} actions',
    lastRun: 'Dernier passage',
    noAgents: 'Aucun agent sur cette installation',
    noAgentsHint: 'Les agents se recrutent dans un projet, et n’apparaissent ici qu’une fois créés.',
    organisationIntro:
      'Écrite d’avance : un responsable ne peut demander à un membre d’agir que si la ligne existe ici. Une ligne enregistrée est une ligne qui fonctionne.',
    noMandates: 'Personne ne peut rien demander à personne.',
    grantedBy: 'Autorisé par {who}, {when}',
  },
  agents: {
    title: 'Agents',
    intro:
      'Des consignes permanentes, écrites dans vos mots. Un agent regarde à votre place et vous dit ce qu’il a vu.',
    recruit: 'Recruter',
    recruitFirst: 'Recruter mon premier agent',
    recruitTitle: 'Recruter un agent',
    editTitle: 'Modifier l’agent',
    recruitIntro: 'Donnez-lui un nom et dites-lui ce que vous attendez, comme à quelqu’un qui arrive.',
    nameLabel: 'Son nom',
    namePlaceholder: 'Clémence',
    missionLabel: 'Son travail',
    missionHelp:
      'Écrivez-le en français courant. Rien ici n’est un mot-clé : la consigne part telle quelle.',
    missionPlaceholder:
      'Tu surveilles mes workspaces. Quand l’un ne démarre pas, tu me dis lequel et pourquoi, en une phrase.',
    examplesTitle: 'Par exemple :',
    exampleWorkspaces:
      'Tu surveilles mes workspaces. Quand l’un ne démarre pas, tu me dis lequel et pourquoi, en une phrase.',
    exampleApps: 'Tu vérifies mes applications. Si l’une est arrêtée, tu la relances et tu me le dis.',
    exampleQuiet: 'Une fois par jour, tu fais le tour de mes projets et tu me signales ce qui a changé.',
    scheduleLabel: 'Ses horaires',
    scheduleManual: 'À la demande',
    scheduleHourly: 'Toutes les heures',
    scheduleDaily: 'Une fois par jour',
    mayRestartLabel: 'Il peut relancer une application arrêtée',
    mayRestartHelp:
      'Sans cela il regarde et rapporte, sans rien changer. C’est la seule chose qu’un agent peut modifier, et chaque relance est tracée.',
    mayRestart: 'Peut relancer une application',
    readsOnly: 'Regarde et rapporte',
    hire: 'Recruter',
    hired: '{name} est en poste.',
    updated: 'Modifié.',
    teamsTitle: 'Équipes',
    teamsIntro:
      'Plusieurs agents sur un même sujet. Vous écrivez d’avance qui peut demander quoi à qui, et rien d’autre ne se produit.',
    teamsEmpty:
      'Vos agents travaillent chacun de leur côté. Une équipe leur donne un sujet commun et vous laisse écrire ce qu’ils peuvent se demander.',
    formTeam: 'Former une équipe',
    formFirstTeam: 'Former ma première équipe',
    formTeamTitle: 'Former une équipe',
    formTeamIntro: 'Donnez-lui un nom et dites sur quoi elle travaille.',
    teamNameLabel: 'Son nom',
    teamNamePlaceholder: 'Astreinte',
    teamPurposeLabel: 'Son sujet',
    teamPurposePlaceholder: 'Garder les applications de production en marche pendant la nuit.',
    teamFormed: 'L’équipe {name} est formée.',
    teamMembers: 'Qui en fait partie',
    teamNoMembers: 'Personne pour l’instant. Placez un agent dans cette équipe depuis sa fiche.',
    organisation: 'Qui peut demander quoi',
    organisationHelp:
      'Une ligne enregistrée est une ligne qui fonctionne : si la demande ne pourrait pas aboutir, elle est refusée tout de suite avec sa raison.',
    organisationEmpty:
      'Personne ne peut encore rien demander à personne. Un responsable peut demander à un membre d’agir, une fois que vous l’avez écrit ici.',
    mandateSentence: '{lead} peut demander à {member} de {action}.',
    askActionRestartApp: 'relancer une application arrêtée',
    allowAsk: 'Autoriser une demande',
    allowAskTitle: 'Autoriser une demande',
    allowAskIntro:
      'Choisissez qui demande et à qui. Cela n’accorde aucun droit nouveau : cela dit seulement qui a le droit de le demander.',
    askLeadLabel: 'Qui demande',
    askMemberLabel: 'À qui',
    askActionLabel: 'De faire quoi',
    askWritten: 'C’est écrit.',
    withdraw: 'Retirer',
    withdrawn: 'Retiré.',
    needsALead:
      'Il faut au moins un responsable dans l’équipe. Un agent devient responsable depuis sa fiche.',
    roleObserver: 'Observe et rapporte',
    roleOperator: 'Peut agir',
    roleLead: 'Responsable',
    roleLabel: 'Son rôle dans l’équipe',
    teamLabel: 'Son équipe',
    noTeam: 'Travaille seul',
    edit: 'Modifier',
    dismiss: 'Renvoyer',
    orphansTitle: 'Agents sans projet',
    orphansHint:
      "Ils n'apparaissent sur aucune page de projet et ne s'exécutent plus. Renvoyez-les, ou recréez-les dans un projet.",
    dismissTitle: 'Renvoyer {name} ?',
    dismissBody:
      'Sa consigne et son carnet de travail sont supprimés. Ce qu’il a relancé ou signalé reste dans l’audit de la plateforme.',
    dismissed: '{name} a été renvoyé.',
    ask: 'Demander',
    askPlaceholder: 'Demandez quelque chose à {name}…',
    youAsked: 'Vous :',
    asked: 'Réponse ajoutée au carnet.',
    runNow: 'Faire maintenant',
    atWork: 'Au travail',
    paused: 'En pause',
    never: 'Jamais travaillé',
    justNow: 'À l’instant',
    minutesAgo: 'Il y a {count} min',
    hoursAgo: 'Il y a {count} h',
    daysAgo: 'Il y a {count} j',
    whatTheySaw: 'Ce que {name} a vu',
    nothingToReport: 'Rien à signaler',
    reported: 'Il a quelque chose à dire.',
    failed: 'En échec',
    loadingRuns: 'Lecture du carnet…',
    neverRan: 'Il n’a pas encore travaillé. Lancez-le maintenant, ou attendez ses horaires.',
    emptyLead:
      'Vous n’avez encore recruté personne. Un agent est quelqu’un à qui vous laissez une consigne permanente, écrite comme vous la diriez :',
    actionRestartApp: 'Application relancée',
  },
  aiServices: {
    title: 'Services d’IA',
    full: 'Service complet',
    degraded: 'Mode réduit',
    down: 'Arrêté',
  },
  home: {
    title: 'Bonjour {name}',
    subtitle: 'Reprenez votre travail ou démarrez un nouveau projet.',
    recentProjects: 'Projets récents',
    recentProjectsHint: 'Vos projets les plus récemment actifs.',
    quickActions: 'Démarrer',
    newProject: 'Nouveau projet',
    newProjectHint: 'Créer un espace de travail collaboratif',
    browseCatalog: 'Parcourir le catalogue',
    browseCatalogHint: 'Datasets, sources de données et ontologies',
    platformActivity: 'Activité de la plateforme',
    users: 'Utilisateurs',
    usersHint: 'Comptes actifs',
    projects: 'Projets',
    projectsHint: 'Espaces collaboratifs',
    workloads: 'Exécutions',
    workloadsHint: 'Workspaces, jobs et applications actifs',
    storage: 'Stockage',
    storagePartial: 'Sur {measured} datasets. {pending} pas encore mesurés : le volume réel est plus élevé.',
    storagePending: "Pas encore mesuré — la première passe tourne cette nuit.",
    storageAsOf: 'Sur les {count} datasets, mesuré {when}.',
    storageUnreadable: "Sur {measured} datasets. {unreadable} n'a pas pu être lu : à réparer, ce total est incomplet.",
    storageHint: 'Déclarés sur la plateforme',
    noProjects: 'Aucun projet pour le moment',
    noProjectsHint:
      'Un projet regroupe vos workspaces, vos exécutions, vos données et les personnes qui y travaillent.',
  },

  projects: {
    storageSizeLabel: 'Capacité des workspaces',
    storageSizeHint: 'Volume monté sur /mnt dans chaque workspace du projet, conservé entre les redémarrages. Les workspaces déjà lancés gardent leur volume.',
    storageSizeDefault: 'Valeur par défaut de la plateforme',
    title: 'Projets',
    subtitle: 'Un projet regroupe vos workspaces, exécutions, données et collaborateurs.',
    create: 'Nouveau projet',
    createTitle: 'Créer un projet',
    createHint: 'Un nom et une description suffisent pour démarrer. Tout le reste se configure ensuite.',
    nameLabel: 'Nom du projet',
    nameHint: 'Visible par tous les membres. Vous pourrez le changer plus tard.',
    namePlaceholder: 'Analyse des ventes 2026',
    descriptionLabel: 'Description',
    descriptionHint: 'Une phrase sur l’objectif du projet, pour vos collaborateurs.',
    search: 'Rechercher un projet',
    empty: 'Aucun projet',
    emptyHint: 'Créez votre premier projet pour commencer à travailler.',
    lastActivity: 'Dernière activité',
    resources: 'Ressources actives',
    openProject: 'Ouvrir le projet',
    deleteTitle: 'Supprimer le projet',
    deleteWarning:
      'Cette action est irréversible. Les workspaces, jobs et applications du projet seront arrêtés et supprimés.',
    transferOwnership: 'Transférer la propriété',
    ownedByUser: 'Appartient à une personne',
    ownedByOrganization: 'Appartient à une organisation',
    ownerTypeLabel: 'Type de propriétaire',
    ownerLabel: 'Nouveau propriétaire',
    viewMode: 'Affichage',
    viewGrid: 'Tuiles',
    viewList: 'Liste',
    createdBy: 'Créé par',
  },

  projectOverview: {
    title: 'Vue d’ensemble',
    subtitle: 'État courant du projet et raccourcis vers ses ressources.',
    activeWorkspaces: 'Workspaces actifs',
    runningJobs: 'Jobs en cours',
    runningApps: 'Applications actives',
    attachedDatasets: 'Datasets rattachés',
    quickLaunch: 'Démarrer un workspace',
    recentActivity: 'Activité récente',
    noActivity: 'Aucune activité récente sur ce projet.',
    settingsTitle: 'Paramètres du projet',
    settingsHint: 'Nom, description et propriétaire du projet.',
  },

  workspaces: {
    assistantSendsDataOffSite:
      "Ce projet monte des données de santé ({datasets}). L'assistant lit les fichiers du workspace pour vous répondre, et ce qu'il lit est envoyé au modèle, hébergé hors du site.",
    startupTitle: 'Démarrage en cours',
    startupHint: 'Les grandes étapes du lancement. Cet écran disparaît dès que l’espace de travail est ouvert.',
    startupStuck: 'Le démarrage est arrêté : il ne repartira pas tout seul.',
    startupTechnical: 'Détail technique',
    step_requested: 'Demande enregistrée',
    step_scheduled: 'Machine réservée',
    step_image: 'Environnement récupéré',
    step_storage: 'Données rattachées',
    step_environment: 'Préparation de la session',
    reason_no_capacity: 'Aucune machine ne peut accueillir cette taille pour le moment.',
    reason_image_unavailable: 'L’image de l’environnement n’a pas pu être récupérée.',
    reason_storage_unavailable: 'Le stockage des données n’a pas pu être rattaché. Prévenez votre administrateur.',
    reason_environment_failed: 'La session s’est arrêtée pendant sa préparation.',
    reason_environment_restarting: 'La session redémarre en boucle.',
    reason_environment_out_of_memory:
      'Fermée pour cause de manque de mémoire. Relancez sur une taille de machine plus grande.',
    title: 'Workspaces',
    subtitle: 'Vos environnements de travail interactifs sur ce projet.',
    create: 'Nouveau workspace',
    createTitle: 'Lancer un workspace',
    createHint: 'Choisissez un environnement et une taille de machine. Le workspace démarre en quelques secondes.',
    environmentLabel: 'Environnement',
    environmentHint: 'Détermine les bibliothèques disponibles et l’IDE utilisé.',
    ideLabel: 'IDE',
    ideHint: 'Déterminé automatiquement par l’environnement choisi.',
    tierLabel: 'Taille de machine',
    tierHint: 'Vous pourrez relancer le workspace sur une taille différente à tout moment.',
    nameLabel: 'Nom du workspace',
    nameHint: 'Pour vous y retrouver si vous en lancez plusieurs.',
    namePlaceholder: 'Exploration des ventes',
    storageLabel: 'Espace de travail persistant',
    storageHint: 'Volume monté sur /mnt, conservé entre les redémarrages.',
    empty: 'Aucun workspace',
    emptyHint:
      'Un workspace est un environnement de développement prêt à l’emploi : notebook, éditeur et données du projet déjà montées.',
    open: 'Ouvrir',
    openPending: 'Démarrage en cours…',
    stopTitle: 'Arrêter le workspace',
    stopWarning:
      'Le contenu de /mnt est conservé. Les processus en cours et le contenu hors /mnt seront perdus.',
    stop: 'Arrêter',
    launched: 'Workspace lancé. Il sera accessible dès qu’il aura démarré.',
    stopped: 'Workspace arrêté.',
    notReady: 'Le workspace n’est pas encore prêt.',
    resources: 'Ressources',
  },

  jobs: {
    title: 'Jobs',
    subtitle: 'Exécutions ponctuelles et planifiées de ce projet.',
    create: 'Nouveau job',
    createTitle: 'Lancer un job',
    createHint: 'Un job exécute une commande dans un environnement, une fois ou selon une planification.',
    nameLabel: 'Nom du job',
    namePlaceholder: 'Rafraîchissement quotidien',
    commandLabel: 'Commande à exécuter',
    commandHint: 'Exécutée à la racine du projet, avec /mnt et les datasets déjà montés.',
    commandPlaceholder: 'python scripts/refresh.py',
    modeLabel: 'Déclenchement',
    modeOnce: 'Une seule fois, maintenant',
    modeSchedule: 'Selon une planification',
    scheduleLabel: 'Planification',
    scheduleHint: 'Format cron à 5 champs : minute, heure, jour du mois, mois, jour de la semaine.',
    timeZoneLabel: 'Fuseau horaire',
    history: 'Historique des exécutions',
    historyHint: 'Les 200 exécutions les plus récentes.',
    schedules: 'Planifications',
    schedulesHint: 'Deux exécutions d’une même planification ne peuvent pas se chevaucher.',
    empty: 'Aucune exécution',
    emptyHint: 'Lancez un job pour exécuter un script sur les ressources du projet.',
    emptySchedules: 'Aucune planification',
    emptySchedulesHint: 'Planifiez un job pour l’exécuter automatiquement, par exemple chaque matin.',
    result: 'Résultat',
    completedAt: 'Terminé',
    nextRun: 'Prochaine exécution',
    lastRun: 'Dernière exécution',
    deleteTitle: 'Supprimer le job',
    deleteWarning: 'L’exécution en cours sera interrompue et son historique supprimé.',
    launched: 'Job lancé.',
    scheduled: 'Planification enregistrée.',
    everyDayAt: 'chaque jour à {time}',
    everyWeekdayAt: 'du lundi au vendredi à {time}',
    everyHour: 'toutes les heures',
    customSchedule: 'planification personnalisée',
  },

  apps: {
    usage: 'Consultation',
    usageHint: 'Qui a consulté cette application, sur les {days} derniers jours.',
    usageViews: 'Consultations',
    usageVisitors: 'Visiteurs identifiés',
    usageLastView: 'Dernière consultation',
    usageAnonymous: 'dont {count} anonymes',
    usageEmpty: 'Personne n’a encore consulté cette application.',
    logsStarting: 'L’application démarre : ses dépendances s’installent. Les journaux arrivent.',
    title: 'Applications',
    subtitle: 'Applications web servies depuis ce projet.',
    create: 'Nouvelle application',
    createTitle: 'Créer une application',
    createHint: 'Choisissez un framework : la commande de lancement est générée pour vous.',
    frameworkLabel: 'Framework',
    frameworkHint: 'Détermine la commande de lancement et le port par défaut.',
    nameLabel: 'Nom de l’application',
    namePlaceholder: 'Tableau de bord commercial',
    slugLabel: 'Adresse publique',
    slugHint: 'Utilisée dans l’URL de l’application. Lettres minuscules, chiffres et tirets.',
    entrypointLabel: 'Fichier principal',
    entrypointHint: 'Chemin relatif à la racine du projet.',
    portLabel: 'Port',
    portHint: 'Port sur lequel l’application écoute. Renseigné automatiquement selon le framework.',
    commandLabel: 'Commande de lancement',
    commandHint: 'Générée à partir du framework. Modifiable si votre application a besoin d’options particulières.',
    accessLabel: 'Qui peut accéder à l’application',
    accessPrivate: 'Membres du projet uniquement',
    accessOrganization: 'Toute mon organisation',
    accessPublic: 'Toute personne authentifiée',
    empty: 'Aucune application',
    emptyHint:
      'Publiez une application Streamlit, Dash ou Shiny pour partager vos résultats sans donner accès au code.',
    restart: 'Redémarrer',
    stop: 'Arrêter',
    publish: 'Publier en production',
    publishTitle: 'Publier l’application',
    publishHint:
      'Une révision figée est créée et servie en production. Vous pourrez revenir à une révision précédente à tout moment.',
    revisions: 'Révisions',
    revisionsHint: 'Chaque publication crée une révision restaurable.',
    rollback: 'Restaurer cette révision',
    rollbackTitle: 'Restaurer la révision {number}',
    rollbackWarning: 'La révision actuellement publiée sera remplacée. Aucune donnée n’est perdue.',
    health: 'Santé',
    restarts: 'Redémarrages',
    deleteTitle: 'Supprimer l’application',
    deleteWarning: 'L’application sera arrêtée et son adresse publique libérée.',
  },

  apis: {
    title: 'API',
    subtitle: 'Des points d’appel déployés depuis vos projets, consommés par d’autres applications du système d’information.',
    create: 'Déployer une API',
    empty: 'Aucune API déployée',
    emptyHint: 'Une API est la même charge qu’une application, mais personne ne l’ouvre dans un navigateur : la plateforme affiche donc un exemple d’appel plutôt qu’un aperçu.',
    callTitle: 'Appeler cette API',
    callHint: 'L’adresse, les extraits prêts à coller, et un jeton qui appartient au projet — il survit au départ de celui qui l’a créé.',
    endpoint: 'Adresse',
    tokenName: 'Nom du jeton',
    tokenNameHint: 'Le nom sert à le révoquer sans deviner : « scoring-prod » vaut mieux que « jeton 3 ».',
    tokenCreate: 'Créer un jeton',
    tokenRevoke: 'Révoquer',
    tokenOnce: 'Ce jeton est affiché une seule fois et n’est pas récupérable. Copiez-le maintenant.',
    tokenLastUsed: 'utilisé {when}',
    tokenNeverUsed: 'jamais utilisé',
  },
  dashboards: {
    title: 'Dashboards',
    subtitle: 'Tableaux de bord publiés avec leur propre adresse.',
    create: 'Nouveau dashboard',
    createTitle: 'Créer un dashboard',
    createHint:
      'Un dashboard est une application dédiée à la visualisation, avec une adresse stable que vous pouvez partager.',
    empty: 'Aucun dashboard',
    emptyHint: 'Créez un dashboard pour partager des résultats avec des personnes qui n’ouvrent pas de notebook.',
  },

  environments: {
    buildProject: 'Projet de construction',
    buildProjectHint: 'L’image est construite dans ce projet et consomme son quota. L’environnement, lui, reste utilisable par tous les projets auxquels vous avez accès.',
    buildProjectPlaceholder: 'Choisissez un projet',
    title: 'Environnements',
    subtitle: 'Images reproductibles utilisées par vos workspaces, jobs et applications.',
    create: 'Nouvel environnement',
    createTitle: 'Créer un environnement',
    createHint: 'Partez d’un environnement existant ou d’une définition vierge, puis ajoutez vos bibliothèques.',
    nameLabel: 'Nom de l’environnement',
    namePlaceholder: 'analyse-python',
    nameHint: 'Lettres minuscules, chiffres et tirets.',
    baseLabel: 'Point de départ',
    baseEmpty: 'Définition vierge',
    baseHint: 'Dupliquer un environnement existant conserve ses bibliothèques.',
    definition: 'Définition',
    definitionHint: 'Le fichier qui décrit les bibliothèques installées dans l’environnement.',
    build: 'Construire une révision',
    building: 'Construction en cours',
    revisions: 'Révisions',
    deleteRevision: 'Supprimer cette révision',
    deleteRevisionHint: "Retire {image} de l'historique de cet environnement. L'image reste dans le registre.",
    revisionDeleted: 'Révision supprimée',
    systemRebuildHint: "Cet environnement est celui que la plateforme est configurée pour faire tourner. Le reconstruire pousserait dans le dépôt de la plateforme, qu'elle ne démarrera pas — construisez sous un nom à vous.",
    revisionsHint: 'Chaque construction produit une révision utilisable.',
    vulnerabilities: 'Vulnérabilités',
    noVulnerabilities: 'Aucune',
    notScanned: 'Non scanné',
    scanUnavailable: 'Inconnu',
    provenance: 'Source',
    provenanceUnknown: 'Inconnue',
    revisionNumber: 'Révision {number}',
    activeRevision: 'Révision active',
    empty: 'Aucun environnement',
    emptyHint:
      'Un environnement fige les bibliothèques dont votre équipe a besoin, pour que tout le monde exécute le même code.',
    ides: 'IDE disponibles',
    imageSize: 'Taille de l’image',
    cleanupCanceled: 'Nettoyer les constructions annulées',
    deleteTitle: 'Supprimer l’environnement',
    deleteWarning:
      'Les workspaces et jobs qui l’utilisent ne pourront plus démarrer. Les exécutions en cours ne sont pas interrompues.',
    forkHint: 'Ajoutez ici ce que cet environnement ajoute à l’image de base, par exemple : RUN pip install --no-cache-dir pandas',
    buildOutput: 'Sortie de la construction',
    buildRunning: 'construction en cours…',
    buildStarted: 'Construction lancée. Vous pouvez suivre son avancement ici.',
    technicalDetails: 'Détails techniques',
  },

  datasets: {
    title: 'Datasets',
    subtitle: 'Jeux de données stockés sur S3 et partagés entre projets.',
    create: 'Nouveau dataset',
    createTitle: 'Créer un dataset',
    createHint: 'Créez un espace de stockage géré par la plateforme, ou rattachez un bucket S3 existant.',
    modeLabel: 'Type de stockage',
    modeLocal: 'Stockage géré par la plateforme',
    modeLocalHint: 'La plateforme crée et gère le bucket pour vous.',
    modeExternal: 'Bucket S3 existant',
    modeExternalHint: 'Vous fournissez l’adresse et des identifiants d’accès.',
    classificationLabel: 'Classification',
    classificationStandard: 'Standard',
    classificationHds: 'Données de santé (HDS)',
    classificationHint: 'La classification HDS restreint l’aperçu, le téléchargement et l’export.',
    ownerOrganizationLabel: 'Organisation propriétaire',
    ownerOrganizationPlaceholder: 'Choisir une organisation',
    hdsWarning:
      'La création et l’attribution de datasets HDS sont réservées aux administrateurs. L’aperçu est limité et le téléchargement direct est désactivé.',
    endpointLabel: 'Adresse du service S3',
    regionLabel: 'Région',
    bucketLabel: 'Nom du bucket',
    prefixLabel: 'Préfixe',
    prefixHint: 'Restreint le dataset à un sous-dossier du bucket.',
    accessKeyLabel: 'Clé d’accès',
    secretKeyLabel: 'Clé secrète',
    credentialsHint:
      'Les identifiants sont chiffrés et ne sont jamais réaffichés. Utilisez des identifiants dédiés, avec les droits minimum nécessaires.',
    empty: 'Aucun dataset',
    emptyHint: 'Créez un dataset pour stocker vos fichiers et les partager avec votre équipe.',
    files: 'Fichiers',
    filesCount: 'Fichiers',
    inThisFolder: 'Dans ce dossier : {files} fichiers, {size}',
    totalSize: 'Volume total',
    explorer: 'Explorateur de fichiers',
    upload: 'Téléverser',
    uploadTitle: 'Ajouter des fichiers',
    uploadPathLabel: 'Dossier de destination',
    uploadFileLabel: 'Fichier',
    newFolder: 'Nouveau dossier',
    rename: 'Renommer',
    renameNewName: 'Nouveau nom',
    renameHint: "Le fichier reste dans ce dossier. La copie est verifiee avant que l'ancien nom soit retire : en cas d'echec, les deux noms subsistent plutot qu'aucun.",
    objectRenamed: 'Objet renomme',
    folderNameLabel: 'Nom du dossier',
    emptyFolder: 'Ce dossier est vide',
    emptyFolderHint: 'Téléversez un premier fichier pour commencer.',
    parentFolder: 'Dossier parent',
    downloadSelection: 'Télécharger la sélection',
    deleteSelection: 'Supprimer la sélection',
    selectedCount: '{count} élément sélectionné',
    selectedCountPlural: '{count} éléments sélectionnés',
    preview: 'Aperçu',
    permissions: 'Permissions',
    replaceCredentials: 'Remplacer les identifiants',
    replaceCredentialsHint: "Éprouvés sur le bucket avant d'être enregistrés. Des identifiants qui ne l'atteignent pas sont refusés et les actuels restent en place.",
    accessKey: "Clé d'accès",
    secretKey: 'Clé secrète',
    credentialsReplaced: 'Identifiants remplacés',
    ownershipHint: 'Le propriétaire administre le dataset et ses droits. Une organisation survit au départ de la personne qui l’a créé.',
    permissionsHint:
      'Le propriétaire gère la ressource. Writer : lecture et écriture. Reader : lecture seule.',
    grant: 'Accorder un accès',
    revoke: 'Révoquer l’accès',
    storage: 'Stockage',
    classification: 'Classification',
    deleteTitle: 'Supprimer le dataset',
    deleteWarning:
      'Le dataset et ses métadonnées sont supprimés. Pour un bucket externe, les fichiers restent dans votre bucket.',
    deleteObjectsTitle: 'Supprimer les fichiers sélectionnés',
    deleteObjectsWarning: 'Cette action est irréversible.',
    uploadDone: 'Fichier téléversé.',
    uploadFailed: 'Échec du téléversement',
  },

  datasources: {
    title: 'Sources de données',
    subtitle: 'Connexions vers des bases et services de données, rattachables à vos projets.',
    create: 'Nouvelle source',
    createTitle: 'Ajouter une source de données',
    createHint: 'Connectez une base existante, ou déployez un service géré par la plateforme.',
    typeLabel: 'Type de source',
    hostLabel: 'Hôte',
    portLabel: 'Port',
    databaseLabel: 'Base de données',
    usernameLabel: 'Utilisateur',
    passwordSecretLabel: 'Secret contenant le mot de passe',
    passwordSecretHint: 'Sélectionnez un secret existant plutôt que de saisir le mot de passe ici.',
    sslLabel: 'Mode SSL',
    empty: 'Aucune source de données',
    emptyHint: 'Ajoutez une connexion pour interroger une base depuis vos workspaces et vos jobs.',
    test: 'Tester la connexion',
    testOk: 'Connexion établie.',
    testFailed: 'Connexion impossible',
    restart: 'Redémarrer le service',
    attached: 'Projets rattachés',
    attach: 'Rattacher au projet',
    detach: 'Détacher du projet',
    managed: 'Service géré',
    external: 'Connexion externe',
    deleteTitle: 'Supprimer la source de données',
    deleteWarning: 'Les projets rattachés perdront l’accès à cette source.',
  },

  ontologies: {
    title: 'Ontologies',
    subtitle: 'Vos objets métier au-dessus des données brutes. Aujourd’hui, une première ébauche déduite des chemins d’un dataset.',
    empty: 'Aucune ontologie',
    emptyHint: 'Une ontologie décrit vos objets métier au-dessus des données. Noryx en construit une première ébauche en lisant les chemins des objets d’un dataset — jamais leur contenu : études, sujets, visites, modalités, volumes.',
    scanTitle: 'Analyser une source',
    scanHint: 'Le scan lit les chemins des objets du dataset — jamais leur contenu — et en déduit une première ébauche : étude, sujets, visites, modalités, formats, volumes. Il crée une nouvelle ontologie ; il ne remplace pas les existantes.',
    scanProject: 'Projet',
    scanDataset: 'Dataset',
    scanProfile: 'Profil d’inférence',
    scanDone: 'Ontologie créée : {objects} objets, {subjects} sujets.',
    scanNoDataset: 'Aucun dataset attaché à ce projet. Attachez-en un depuis l’onglet Données du projet.',
    scanProfileHint: 'health-file-path-v1 reconnaît <sujet>/<visite>/<modalité> : un sujet comme PREMYOM1000-001, une visite en AAAAMMJJ (préfixe visit_ accepté), la modalité juste après. Ce qui ne suit pas cette forme est compté et décrit, jamais deviné.',
    scan: 'Analyser la source',
    scanning: 'Analyse en cours',
    query: 'Filtrer',
    queryLabel: 'Filtre',
    queryHint: 'Filtrez les objets par sujet, visite ou modalité.',
    object: 'Objet',
    objectType: 'Type',
    parent: 'Rattaché à',
    objects: 'Objets',
    matches: '{shown} résultats affichés sur {total}.',
    noMatch: 'Aucun objet ne correspond à ce filtre.',
    unrecognised: 'Chemins non reconnus',
    unrecognisedHint: 'Objets dont le chemin ne suit pas la convention que ce profil sait lire. Leur forme est indiquée pour que vous puissiez juger : {shapes}',
    queryPlaceholder: 'ANTERION',
    source: 'Source',
    profile: 'Profil d’inférence',
    manifest: 'Manifeste',
    completeness: 'Couverture',
    completenessHint: 'Combien de sujets portent chaque modalité, et lesquels ne l’ont pas. Une cohorte demandée par modalité exclut silencieusement les seconds.',
    completenessSubjects: '{complete} sujets sur {total} portent toutes les modalités de l’étude.',
    completenessModality: 'Modalité',
    completenessHolders: 'Sujets',
    completenessMissing: 'Sujets sans cette modalité',
    completenessNoGap: 'Aucun',
    cohorts: 'Cohortes',
    cohortsHint: 'Une cohorte fige la liste des fichiers au moment où vous la déclarez : la même question posée le mois prochain désigne toujours la même étude. Rien n’est dupliqué — le montage crée un arbre de liens vers le dataset.',
    cohortCreate: 'Déclarer une cohorte',
    cohortName: 'Nom',
    cohortDescription: 'Description',
    cohortModalities: 'Modalités (vide = toutes)',
    cohortSubjects: 'Sujets (vide = tous)',
    cohortFiles: '{count} fichiers',
    cohortEmpty: 'Aucune cohorte pour cette ontologie.',
    cohortCreated: 'Cohorte figée sur {count} fichiers.',
    cohortDeleteTitle: 'Supprimer la cohorte',
    cohortDeleteWarning: 'La sélection figée est perdue. Les fichiers du dataset ne sont pas touchés.',
    cohortMountHint: 'Montée dans les workspaces du projet sous /mnt/cohorts/<nom>/<sujet>/<visite>/<modalité>.',
    freshness: 'Fraîcheur',
    freshnessMeasured: 'Mesurée il y a {days} jours, sur {objects} objets.',
    freshnessMeasuredToday: 'Mesurée aujourd’hui, sur {objects} objets.',
    freshnessAligned: 'La source contient toujours le même nombre d’objets.',
    freshnessGrown: 'La source contient {drift} objets de plus depuis cette mesure.',
    freshnessShrunk: 'La source contient {drift} objets de moins depuis cette mesure.',
    freshnessUnknown: 'Source non consultée : impossible de dire si cette mesure est toujours juste.',
    freshnessStale: 'Cette ontologie ne décrit plus sa source',
    ownershipHint: 'Le propriétaire administre l’ontologie et ses droits. Une organisation survit au départ de la personne qui l’a créée.',
    deleteTitle: 'Supprimer l’ontologie',
    deleteWarning: 'Les projets rattachés perdront l’accès à ce modèle.',
  },

  repositories: {
    title: 'Dépôts Git',
    subtitle: 'Dépôts clonés dans vos workspaces sous /repos.',
    create: 'Nouveau dépôt',
    createTitle: 'Ajouter un dépôt Git',
    createHint: 'Le dépôt est cloné automatiquement dans /repos au démarrage de vos workspaces.',
    urlLabel: 'Adresse du dépôt',
    tokenTooBroad: 'Ce jeton peut aussi :',
    tokenTooBroadDanger: 'Ce jeton peut détruire ou administrer :',
    urlHint: 'Adresse HTTPS du dépôt. L’authentification se fait par jeton ; les adresses SSH ne sont pas encore prises en charge.',
    defaultRefLabel: 'Branche par défaut',
    authTypeLabel: 'Authentification',
    authPersonalToken: 'Jeton personnel (PAT)',
    authRepositoryToken: 'Jeton de dépôt (PRAT)',
    authNone: 'Dépôt public',
    authSecret: 'Jeton stocké dans un secret',
    authSecretLabel: 'Secret contenant le jeton',
    authorNameLabel: 'Nom pour les commits',
    authorEmailLabel: 'Adresse e-mail pour les commits',
    empty: 'Aucun dépôt Git',
    emptyHint: 'Ajoutez un dépôt pour retrouver votre code dans chaque workspace du projet.',
    validate: 'Vérifier l’accès',
    reachable: 'Accessible',
    unreachable: 'Inaccessible',
    lastValidated: 'Dernière vérification',
    deleteTitle: 'Supprimer le dépôt',
    deleteWarning: 'Le dépôt ne sera plus cloné dans les nouveaux workspaces.',
  },

  secrets: {
    title: 'Secrets',
    subtitle: 'Valeurs sensibles injectées dans vos exécutions.',
    create: 'Nouveau secret',
    createTitle: 'Créer un secret',
    createHint: 'La valeur est chiffrée et ne sera plus jamais affichée après enregistrement.',
    nameLabel: 'Nom du secret',
    nameHint: 'Injecté dans vos exécutions sous la forme NORYX_SECRET_{name}.',
    namePlaceholder: 'api_partenaire',
    valueLabel: 'Valeur',
    valueHint: 'Elle ne sera plus affichée après enregistrement.',
    kindLabel: 'Nature du secret',
    kindHint: 'Un jeton a une durée de vie décidée ailleurs ; la plateforme prévient avant qu’il n’expire, à condition de connaître la date.',
    kindGeneric: 'Valeur quelconque',
    kindPersonalToken: 'Jeton personnel (PAT)',
    kindRepositoryToken: 'Jeton de dépôt (PRAT)',
    expiresRequiredHint: 'GitHub et GitLab plafonnent leurs jetons à un an.',
    neverExpires: 'Ce jeton n’expire pas',
    expiryAnswerRequired: 'Indiquez la date d’expiration, ou cochez que le jeton n’expire pas.',
    expiresLabel: 'Date d’expiration',
    expiresHint: 'Optionnelle. Un secret expiré n’est plus injecté dans les exécutions.',
    empty: 'Aucun secret',
    emptyHint: 'Stockez ici vos jetons et mots de passe plutôt que de les écrire dans votre code.',
    expiresAt: 'Expiration',
    expired: 'Expiré',
    expiringSoon: 'Expire bientôt',
    deleteTitle: 'Supprimer le secret',
    deleteWarning: 'Les exécutions qui l’utilisent échoueront au prochain démarrage.',
    created: 'Secret enregistré.',
  },

  production: {
    title: 'Production',
    subtitle: 'Inventaire des services actuellement publiés sur la plateforme.',
    published: 'Services publiés',
    running: 'En marche',
    public: 'Accessibles largement',
    revision: 'Révision',
    access: 'Accès',
    publishedAt: 'Publié le',
    publishedBy: 'Publié par',
    empty: 'Aucun service publié',
    emptyHint: 'Publiez une application depuis un projet pour la voir apparaître ici.',
    revisionHistory: 'Historique des révisions',
    selectService: 'Sélectionnez un service pour voir ses révisions.',
  },

  members: {
    orgTitle: 'Accès par organisation',
    orgHint: 'Chaque membre de l’organisation obtient ce rôle sur le projet. Les accès s’additionnent : c’est le plus fort qui s’applique, entre le rôle personnel et celui hérité d’une organisation.',
    orgEmpty: 'Aucune organisation n’a d’accès à ce projet.',
    orgLabel: 'Organisation',
    orgPlaceholder: 'Choisissez une organisation',
    orgGrant: 'Attribuer',
    orgGranted: 'Accès attribué à l’organisation.',
    title: 'Membres',
    subtitle: 'Personnes ayant accès à ce projet et leur niveau de droits.',
    invite: 'Inviter',
    inviteTitle: 'Inviter une personne',
    inviteHint: 'La personne doit déjà avoir un compte sur la plateforme.',
    userLabel: 'Utilisateur',
    roleLabel: 'Rôle',
    roleViewer: 'Lecteur',
    roleBasedOn: 'Hérite de {base}',
    roleViewerHint: 'Consulte le projet et ses résultats, sans rien modifier.',
    roleEditor: 'Contributeur',
    roleEditorHint: 'Lance des workspaces et des exécutions, modifie les ressources.',
    roleAdmin: 'Administrateur',
    roleAdminHint: 'Gère les membres et les paramètres du projet.',
    empty: 'Aucun membre',
    emptyHint: 'Invitez vos collaborateurs pour travailler ensemble sur ce projet.',
    removeTitle: 'Retirer du projet',
    removeWarning: 'La personne perdra immédiatement l’accès au projet.',
  },

  network: {
    title: 'Réseau et sécurité',
    subtitle: 'Accès Internet sortant demandé par les projets et les utilisateurs.',
    requestAccess: 'Demander un accès',
    requestTitle: 'Demander un accès sortant',
    profileLabel: 'Profil d’accès',
    destinationLabel: 'Destination',
    destinationHint: 'Nom de domaine ou plage d’adresses autorisée.',
    portLabel: 'Port',
    protocolLabel: 'Protocole',
    workloadsLabel: 'Types d’exécution concernés',
    justificationLabel: 'Justification',
    justificationHint: 'Expliquez l’usage. Cette information est conservée pour l’audit.',
    expiresLabel: 'Expiration de l’autorisation',
    approve: 'Approuver',
    reject: 'Refuser',
    decisionNoteLabel: 'Motif de la décision',
    empty: 'Aucune demande',
    emptyHint: 'Les demandes d’accès Internet sortant apparaîtront ici pour validation.',
    pending: 'En attente',
    approved: 'Approuvée',
    rejected: 'Refusée',
    requester: 'Demandeur',
  },

  rbac: {
    title: 'Qui a accès à quoi',
    subtitle: 'Rapport en lecture seule. Chaque ligne indique si l’accès est direct, hérité d’une organisation ou lié à une propriété.',
    matrix: 'Matrice des droits',
    matrixHint: 'Vue consolidée des droits effectifs par sujet et par ressource.',
    subject: 'Sujet',
    resource: 'Ressource',
    empty: 'Aucun droit enregistré',
    emptyHint: 'Les droits accordés sur les projets et les datasets apparaîtront ici.',
    organizations: 'Organisations',
    organizationsHint: 'Un utilisateur sans organisation ne peut pas accéder à la plateforme.',
    createOrganization: 'Créer une organisation',
    organizationNameLabel: 'Nom',
    organizationAliasLabel: 'Alias',
    organizationAliasHint: 'Identifiant court, en minuscules, utilisé par les règles d’accès.',
    members: 'Membres',
    addMember: 'Ajouter un membre',
    noOrganizationSelected: 'Sélectionnez une organisation pour voir ses membres.',
  },

  activity: {
    title: 'Activité',
    subtitle: 'Exécutions en cours sur la plateforme, tous projets confondus.',
    executions: 'Exécutions actives',
    pods: 'Pods',
    cpuReserved: 'CPU réservé',
    memoryReserved: 'Mémoire réservée',
    empty: 'Aucune exécution en cours',
    emptyHint: 'Les workspaces, jobs et applications actifs apparaîtront ici.',
    stopTitle: 'Arrêter l’exécution',
    stopWarning: 'L’exécution sera interrompue immédiatement.',
    kind: 'Type',
  },

  quota: {
    title: 'Quota du projet',
    hint: 'Ce que ce projet peut faire tourner en même temps. Un lancement au-delà est refusé, en nommant la limite atteinte.',
    unlimited: 'Aucune limite définie : ce projet peut lancer autant que le cluster le permet.',
    vcpu: 'vCPU',
    memory: 'Mémoire',
    workspaces: 'Workspaces',
    jobs: 'Traitements',
    noLimit: 'illimité',
    zeroMeansNoLimit: '0 signifie aucune limite sur cette dimension.',
  },
  usage: {
    consumption: 'Consommation',
    consumptionHint: 'Ce que le projet a réellement consommé, mesuré toutes les cinq minutes et conservé cent jours.',
    restsOn: 'Total établi sur {samples} mesures.',
    title: 'Consommation par projet',
    hint: 'Ce que chaque projet a réellement consommé sur les 30 derniers jours. Mesuré par échantillonnage toutes les 5 minutes ; une interruption de la plateforme n’est pas facturée.',
    project: 'Projet',
    vcpuHours: 'vCPU·heures',
    memoryHours: 'Gio·heures',
    peak: 'Pic',
    samples: 'Mesures',
    empty: 'Aucune consommation enregistrée sur la période.',
  },
  people: {
    title: 'Personnes & organisations',
    hint: 'Qui est qui : chaque organisation, ses equipes, et les personnes qui les composent. Tout se modifie depuis le panneau de droite.',
    newOrganization: 'Nouvelle organisation',
    newOrganizationHint: "Une organisation est portee par l'annuaire : on la cree ici, on ne la supprime pas ici.",
    organizationCreated: 'Organisation creee',
    alias: 'Alias',
    organizationSummary: '{people} personne(s), {teams} equipe(s)',
    newTeam: 'Nouvelle equipe',
    addToOrganization: "Ajouter a l'organisation",
    removeFromOrganization: "Retirer de l'organisation",
    teamSummary: '{count} membre(s)',
    teamRenamed: 'Equipe renommee',
    teamRemoveWarning: "Supprimer l'equipe {name} ? Ses {count} membre(s) perdent l'acces qu'elle leur donnait, et ses octrois sur les projets partent avec elle.",
    teamRemoveHint: "{count} membre(s) concerne(s). Les octrois de l'equipe sur les projets sont retires avec elle.",
    unaffiliated: 'Sans organisation',
    unaffiliatedHint: "Des comptes qui n'appartiennent a aucune organisation - typiquement des comptes de service, ou des personnes pas encore rattachees.",
    nobody: 'Personne.',
    deactivated: 'Desactive',
    neverSeen: 'Jamais connecte',
    createUserIn: 'Le compte sera cree dans {organization}.',
  },
  admin: {
    visibleAsAdmin: 'admin',
    visibleAsAdminHint: 'Visible parce que vous administrez la plateforme : vous n’avez aucun droit propre sur cet objet.',
    deleteAccount: 'Supprimer le compte',
    deleteHint: 'La suppression retire définitivement le compte du fournisseur d’identité. Elle n’est pas réversible : réactiver ne sera plus possible.',
    deleteSecrets: 'Ses secrets personnels sont supprimés : ils appartiennent à une personne, pas à un rôle, et ne s’héritent pas.',
    deleteAudit: 'La piste d’audit conserve les événements produits par ce compte, sous son identifiant.',
    deletedHint: 'Compte supprimé, {count} secret(s) personnel(s) effacé(s).',
    deactivate: 'Désactiver le compte',
    deactivateHint: 'Le compte ne pourra plus se connecter. Il n’est pas supprimé : la piste d’audit reste attribuable, et le compte peut être réactivé.',
    deactivateTokens: 'Ses jetons d’API sont révoqués — un jeton agit comme son porteur, un compte désactivé dont les jetons fonctionnent encore ne l’est pas.',
    deactivateSecrets: 'Ses secrets personnels ne sont pas transférés : ils appartiennent à la personne, pas au rôle.',
    deactivatedHint: 'Compte désactivé, {count} jeton(s) révoqué(s)',
    reactivate: 'Réactiver',
    ownedTitle: 'Ce que ce compte possède',
    ownsNothing: 'Ce compte ne possède rien en propre : rien à transférer.',
    ownedProjects: 'Projets :',
    ownedDatasets: 'Jeux de données :',
    ownedOntologies: 'Ontologies :',
    ownedApps: 'Applications :',
    ownedDatasources: 'Sources de données :',
    ownedRepositories: 'Dépôts :',
    successorLabel: 'Repreneur',
    successorHint: 'Recevra la propriété de tout ce qui précède, dans la même opération.',
    smtp: 'Serveur de messagerie',
    smtpHint: 'Utilisé par la plateforme pour envoyer les liens d’invitation et de réinitialisation. Renseigné par installation : chaque site passe par son propre relais.',
    smtpConfigured: 'Configuré',
    smtpMissing: 'Absent',
    smtpHost: 'Hôte',
    smtpPort: 'Port',
    smtpFrom: 'Adresse d’expédition',
    smtpFromHint: 'Ce que verra le destinataire. Le relais doit accepter cette adresse.',
    smtpFromName: 'Nom affiché',
    smtpReplyTo: 'Répondre à',
    smtpUser: 'Identifiant',
    smtpPassword: 'Mot de passe',
    smtpPasswordHint: 'Écrit seulement : il n’est jamais renvoyé à un navigateur.',
    smtpPasswordStored: 'Un mot de passe est enregistré. Laisser vide pour le conserver.',
    smtpNoAuthHint: 'Authentification désactivée : le relais accepte l’envoi sans identifiants.',
    smtpAuth: 'Authentification',
    smtpTest: 'Envoyer un test',
    smtpTestTo: 'Destinataire du test',
    smtpTestHint: 'Testez avant d’enregistrer : la configuration en place n’est pas remplacée.',
    smtpTestSent: 'Message de test envoyé à {recipient}',
    resetByEmail: 'Envoyer un lien',
    activity: 'Activité',
    activityHint: "Qui s'est servi de la plateforme, et pour quoi. Compté sur ce que la plateforme a enregistré, pas sur ce dont on se souvient.",
    activityEmpty: 'Rien d\'enregistré sur cette période.',
    activityCoverage: "Les enregistrements commencent le {since} : la période antérieure n'est pas calme, elle n'est pas enregistrée.",
    byOrganization: 'Par organisation',
    noOrganization: 'Sans organisation',
    activePeoplePerDay: 'Personnes actives par jour',
    whoWasActive: '{count} personnes actives',
    whatWasDone: 'Ce qui a été fait',
    peopleCount: '{count} personnes',
    eventsCount: '{count} actions',
    actionsTruncated: 'Les {shown} actions les plus fréquentes sur {total} types.',
    window7d: '7 derniers jours',
    window30d: '30 derniers jours',
    window90d: '90 derniers jours',
    administrator: 'Admin',
    administratorHint: "Peut agir sur le travail de tout le monde sur cette plateforme.",
    organization: 'Organisation',
    lastSeen: 'Dernière connexion',
    neverSignedIn: 'Jamais connecté',
    resetByEmailHint: 'Envoie à {user} un lien pour choisir son mot de passe, valable {hours} heures. Rien à dicter ni à coller.',
    resetByEmailSent: 'Lien envoyé',
    teamUsage: 'Consommation par équipe',
    teamUsageHint: 'Sur les 30 derniers jours. La consommation est échantillonnée par projet : le chiffre d\'une équipe est une attribution, pas une mesure.',
    teamUsageEmpty: 'Aucun rapport de consommation disponible.',
    teamUsageNoTeams: 'Aucune équipe n\'atteint de projet : tout est non attribué.',
    teamUsagePlatform: 'Plateforme (vCPU·h)',
    teamUsagePlatformHint: 'Ce que le cluster a réellement consommé.',
    teamUsageAttributed: 'Attribué (vCPU·h)',
    teamUsageAttributedHint: 'La somme des lignes ci-dessous.',
    teamUsageOverlap: 'Dépasse la plateforme de {excess} : des équipes partagent des projets.',
    teamUsageUnattributed: 'Non attribué (vCPU·h)',
    teamUsageUnattributedHint: 'Atteint par aucune équipe — détenu en propre ou par une organisation.',
    teamUsageMembers: 'Membres',
    teamUsageProjects: 'Projets',
    teamUsageVcpuHours: 'vCPU·h',
    teamUsagePeak: 'Pic vCPU',
    teamUsageShared: '{count} partagé(s)',
    teams: 'Équipes',
    teamsHint: 'Des groupes de personnes dans une organisation. Ce qu\'une équipe atteint s\'accorde depuis le projet, pas ici.',
    teamsPickOrganization: 'Choisissez une organisation pour voir ses équipes.',
    teamsEmpty: 'Aucune équipe dans cette organisation.',
    teamNamePlaceholder: 'P&C, Sinistres, Data Science…',
    teamName: 'Nom de l\'équipe',
    teamAdd: 'Ajouter une équipe',
    teamCreated: 'Équipe créée',
    teamRemove: 'Supprimer l\'équipe',
    teamRemoved: 'Équipe supprimée',
    teamRemoveWarning: 'Supprimer {name} retire aussi tous les accès aux projets qu\'elle accordait.',
    teamMemberCount: '{count} membre(s)',
    teamMemberAdd: 'Ajouter un membre',
    teamMemberPlaceholder: 'Choisir une personne',
    teamMemberAdded: 'Membre ajouté',
    teamMemberRemove: 'Retirer de l\'équipe',
    teamNoMembers: 'Personne dans cette équipe.',
    teamMemberRemoved: 'Membre retiré',
    hardwareTiers: 'Gammes de machines',
    hardwareTiersHint: 'Les tailles proposées au lancement d’un workspace, d’un job ou d’une application.',
    tierName: 'Nom affiché',
    tierId: 'Identifiant',
    tierIdHint: 'Sert de référence dans l’API et les workspaces existants : il ne change pas.',
    tierRequest: 'Réservé',
    tierLimit: 'Plafond',
    tierRequestHint: 'Ce que l’ordonnanceur met de côté pour la machine.',
    tierLimitHint: 'Ce que la machine peut atteindre.',
    tierDefault: 'Par défaut',
    tierAdd: 'Ajouter une gamme',
    tierSaved: 'Gamme enregistrée',
    tierRemoved: 'Gamme retirée',
    tierRemoveConfirm: 'Retirer cette gamme ? Les workspaces déjà lancés continuent de tourner ; elle disparaît simplement des choix proposés.',
    tierRunningUnaffected: 'Modifier une gamme n’a pas d’effet sur les workspaces déjà lancés : le changement s’applique aux prochains démarrages.',
    title: 'Administration',
    subtitle: 'Supervision et configuration de la plateforme.',
    overview: 'Vue d’ensemble',
    users: 'Utilisateurs',
    usersHint: 'Comptes connus de la plateforme.',
    inventory: 'Inventaire',
    modules: 'Modules',
    dataUsage: 'Cartographie des données',
    dataUsageHint:
      'Qui possède les datasets, qui y a accès, quels projets les consomment et quelles exécutions y sont rattachées.',
    exportCsv: 'Exporter en CSV',
    audit: 'Audit',
    auditHint: 'Journal des actions réalisées sur la plateforme.',
    auditEmpty: 'Aucun événement',
    auditEmptyHint: 'Les actions réalisées sur la plateforme seront enregistrées ici.',
    actor: 'Auteur',
    action: 'Action',
    outcome: 'Résultat',
    occurredAt: 'Date',
    storageEndpoints: 'Points de stockage',
    storageEndpointsHint: 'Services S3 déclarés pour les datasets et les sauvegardes.',
    addEndpoint: 'Ajouter un point de stockage',
    testEndpoint: 'Tester',
    backups: 'Sauvegardes',
    backupsHint: 'Cible S3 externe utilisée pour les sauvegardes de la plateforme.',
    backupConfigure: 'Configurer la cible',
    backupRun: 'Lancer une sauvegarde',
    backupHistory: 'Historique',
    backupEmpty: 'Aucune sauvegarde',
    backupEmptyHint: 'Configurez une cible S3 puis lancez une première sauvegarde.',
    backupIncomplete: 'La dernière sauvegarde s’est terminée avec des réserves. Son contenu est incomplet.',
    backupNotConfigured: 'Aucune cible configurée',
    backupConfigured: 'Cible configurée',
    bucket: 'Bucket',
    prefix: 'Préfixe',
    endpoint: 'Adresse',
    region: 'Région',
    accessKey: 'Clé d’accès',
    secretKey: 'Clé secrète',
    encryptionKey: 'Clé de chiffrement',
    report: 'Rapport',
    lastUpdated: 'Dernière mise à jour',
  },


  graph: {
    title: 'Graphe des accès',
    focusSubject: 'Centrer sur une personne ou une organisation',
    focusResource: 'Centrer sur une ressource',
    resourceType: 'Type de ressource',
    everyone: 'Tout le monde',
    everything: 'Toutes les ressources',
    allTypes: 'Tous les types',
    showInherited: 'Afficher les accès hérités',
    hideInherited: 'Masquer les accès hérités',
    clear: 'Réinitialiser',
    empty: 'Aucun accès ne correspond à ce filtre.',
    tooMany: '{count} accès à afficher.',
    tooManyHint: 'Choisissez une personne, une ressource ou un type : au-delà, le graphe devient illisible et ne répond plus à rien.',
    legendDirect: 'Accès direct',
    legendInherited: 'Hérité d’une organisation',
    legendOwner: 'Propriétaire',
    edges: '{count} accès affichés',
  },

  inventory: {
    title: 'Logiciels et licences',
    subtitle: 'Ce que la plateforme embarque, avec la licence de chaque composant. Généré depuis le graphe de dépendances réel, pas tenu à la main.',
    component: 'Composant',
    version: 'Version',
    licence: 'Licence',
    partOf: 'Fait partie de',
    source: 'Origine',
    origin_detected: 'Lue',
    origin_declared: 'Déclarée',
    origin_unresolved: 'Non résolue',
    count: '{count} composants',
    unknownCount: '{count} licence(s) non déterminée(s)',
    allKnown: 'Toutes les licences sont déterminées',
    generated: 'Généré le',
  },

  identity: {
    createUser: 'Créer un compte',
    createUserHint: 'Le compte est créé dans Keycloak avec un mot de passe temporaire, que l’utilisateur devra changer à sa première connexion.',
    username: 'Identifiant',
    firstName: 'Prénom',
    lastName: 'Nom',
    organization: 'Organisation',
    organizationHint: 'Sur une installation Enterprise, l’appartenance est obligatoire : sans organisation, le compte se connecte et ne peut rien faire.',
    organizationPlaceholder: 'Choisissez une organisation',
    resetPassword: 'Réinitialiser',
    resetPasswordHint: 'Un nouveau mot de passe temporaire sera généré pour {user}. L’ancien cessera immédiatement de fonctionner.',
    passwordFor: 'Mot de passe temporaire de {user}',
    passwordHint: 'Affiché une seule fois et non récupérable. Transmettez-le, il devra être changé à la première connexion.',
    passwordCopied: 'Mot de passe copié.',
  },

  adminNav: {

    people: 'Personnes et accès',

    resources: 'Ressources',

    operations: 'Exploitation',

    compliance: 'Conformité',

    configuration: 'Configuration',

  },


  projectVariables: {

    title: 'Variables d’environnement',

    hint: 'Les réglages dont le travail a besoin, partagés par tous les membres du projet et injectés dans les workloads sous leur propre nom. Pour un identifiant personnel, utilisez un secret : il vous suit, celles-ci suivent le projet.',

    add: 'Ajouter une variable',

    nameColumn: 'Nom',

    nameHint: 'Lettres, chiffres et tirets bas, comme dans un shell. Les noms commençant par NORYX_ sont réservés à la plateforme.',

    valueColumn: 'Valeur',

    descriptionColumn: 'Description',

    descriptionHint: 'À quoi elle sert, pour celui qui la lira dans six mois.',

    updatedColumn: 'Modifiée',

    hidden: 'Masquée',

    saved: 'Variable enregistrée.',

    removed: 'Variable supprimée.',

    emptyTitle: 'Aucune variable',

    emptyHint: 'Ajoutez l’adresse d’un serveur de suivi, un nom de bucket, une URL de registre — ce que vos workloads attendent dans leur environnement.',

    formHint: 'La valeur est chiffrée au repos et lisible par les membres qui peuvent lancer des workloads.',

    removeTitle: 'Supprimer la variable',

    removeHint: '{name} ne sera plus injectée dans les workloads de ce projet.',

  },


  account: {
    title: 'Mon compte',
    subtitle: 'Vos identifiants d’accès à l’API, vos secrets et vos préférences.',
  },

  tokens: {
    scopeLabel: 'Portée',
    scopeHint: 'Ce que le jeton pourra faire, dans la limite de vos propres droits.',
    scopeRead: 'Lecture seule',
    scopeReadHint: 'Consulter, ne rien modifier.',
    scopeDatasets: 'Datasets',
    scopeDatasetsHint: "Deposer et remplacer des fichiers dans les datasets accessibles. Ne lance rien et ne supprime aucun projet.",
    scopeWorkspaces: 'Workspaces',
    scopeWorkspacesHint: 'Démarrer et arrêter des workspaces, plus la lecture.',
    scopeJobs: 'Traitements',
    scopeJobsHint: 'Lancer jobs et builds — le cas d’une intégration continue.',
    scopeFull: 'Tous vos droits',
    scopeFullHint: 'Aucune restriction : le jeton peut tout ce que vous pouvez.',
    scopeColumn: 'Portée',
    title: 'Jetons d’API',
    subtitle: 'Pour appeler l’API hors navigateur : intégration continue, notebook, script.',
    createTitle: 'Créer un jeton',
    createHint: 'Le jeton agit en votre nom et n’a aucun droit propre : il ne peut rien faire que vous ne puissiez déjà faire.',
    nameLabel: 'Nom',
    nameHint: 'Ce qui vous permettra de révoquer le bon plus tard.',
    namePlaceholder: 'gitlab-ci',
    expiryLabel: 'Expiration',
    expiryDays: '{days} jours',
    expiryNever: 'Sans expiration',
    create: 'Créer',
    issuedTitle: 'Votre jeton',
    issuedHint: 'Il n’est affiché qu’une fois et n’est pas récupérable. Copiez-le maintenant.',
    copy: 'Copier',
    copied: 'Jeton copié.',
    dismiss: 'J’ai copié',
    empty: 'Aucun jeton pour l’instant.',
    created: 'Créé le',
    expires: 'expire le',
    lastUsed: 'utilisé',
    neverUsed: 'jamais utilisé',
    revoke: 'Révoquer',
    revoked: 'Révoqué',
    expired: 'Expiré',
  },

  health: {
    history: 'Historique',
    historyHint: 'Conditions techniques que la plateforme a constatées elle-même. Les échecs de jobs n’y figurent pas : ils appartiennent à qui les a lancés.',
    historyEmpty: 'Aucune condition enregistrée sur cette période.',
    notRecording: 'L’historique n’est pas enregistré sur cette installation.',
    ongoing: 'En cours',
    resolvedAfter: 'Résolue après',
    raised: 'Levée',
    last30: '30 jours',
    last90: '90 jours',
    last365: '1 an',
    title: 'Santé de la plateforme',
    subtitle: 'Conditions nécessitant une intervention.',
    checkedAt: 'Vérifié',
    statusHealthy: 'Tout va bien',
    statusDegraded: 'Dégradée',
    statusCritical: 'Critique',
    allClear: 'Aucun problème détecté.',
    indicator: 'Alertes plateforme',
  },
  settings: {
    title: 'Réglages de la plateforme',
    subtitle: 'Paramètres modifiables sans redéploiement.',
    precedence: 'Un réglage défini ici prime sur la variable d’environnement, qui prime sur la valeur par défaut. Vider un champ rétablit la valeur héritée.',
    saved: 'Réglage enregistré.',
    unset: 'Non défini',
    key_workspace_max_lifetime: 'Durée de vie maximale des workspaces',
    key_alert_webhook_url: 'Webhook d’alerte',
    key_alert_instance_name: 'Nom de l’instance',
    key_ui_default_theme: 'Thème par défaut',
    key_platform_backend_version: 'Version du backend',
    key_platform_edition: 'Édition',
    key_platform_namespace: 'Namespace Kubernetes',
  },
  search: {
    title: 'Recherche',
    open: 'Rechercher',
    placeholder: 'Rechercher…',
    hint: 'Saisissez au moins deux caractères.',
    empty: 'Aucun résultat.',
    kind_project: 'Projet',
    kind_dataset: 'Dataset',
    kind_datasource: 'Source',
    kind_ontology: 'Ontologie',
    kind_repository: 'Dépôt',
    kind_secret: 'Secret',
    kind_workspace: 'Workspace',
    kind_job: 'Job',
    kind_app: 'Application',
  },
  errors: {
    notFoundTitle: 'Page introuvable',
    notFoundHint: 'Le lien que vous avez suivi n’existe pas ou n’existe plus.',
    backHome: 'Retour à l’accueil',
    crashTitle: 'Une erreur inattendue s’est produite',
    crashHint:
      'L’interface a rencontré un problème. Rechargez la page ; si le problème persiste, contactez votre administrateur.',
    reload: 'Recharger la page',
    projectNotFound: 'Projet introuvable',
    projectNotFoundHint: 'Ce projet n’existe plus ou vous n’y avez plus accès.',
    sessionExpiredTitle: 'Votre session a expiré',
    sessionExpiredHint: 'Reconnectez-vous pour reprendre. Le travail en cours dans vos espaces n’est pas affecté : ils continuent de tourner.',
    signedOutTitle: 'Session expirée',
    signedOutHint: 'Reconnectez-vous pour reprendre votre travail.',
    noOrganizationTitle: 'Une organisation est requise',
    noOrganizationHint:
      'Votre compte est authentifié mais n’est rattaché à aucune organisation. Contactez un administrateur de la plateforme.',
  },
} as const;

export type Catalogue = typeof fr;
