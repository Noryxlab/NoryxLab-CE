	...
	stmts := []string{
		...
		`INSERT INTO ontologies (id, owner_user_id, owner_type, owner_id, name, description, source_type, source_id, source_name, inference_profile, status, manifest_json, created_at, updated_at)
		 SELECT po.project_id,
		        ...
		        COALESCE(NULLIF(po.manifest_json->>'inferenceProfile', ''), CASE WHEN po.manifest_json->>'sourceType'='datasource' THEN 'datasource-metadata-v1' ELSE 'noryx-file-path-v1' END),
		        'draft',
		        po.manifest_json,
		        po.generated_at,
		        po.generated_at
