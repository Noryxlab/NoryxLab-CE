...
func (s *APITokenStore) Put(token apitoken.Token) error {
	...
	_, err := s.Store.db.ExecContext(ctx, `
		INSERT INTO api_tokens (id, user_id, name, secret_hash, created_at, expires_at, scopes, project_id, component)
		VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`,
		token.ID, token.UserID, token.Name, token.SecretHash, token.CreatedAt.UTC(), token.ExpiresAt,
		strings.Join(token.Scopes, ","), token.ProjectID, token.Component)
	return err
}
...
func (s *APITokenStore) Get(id string) (apitoken.Token, bool, error) {
	...
	row := s.Store.db.QueryRowContext(ctx, `
		SELECT id, user_id, name, secret_hash, created_at, expires_at, revoked_at, last_used_at, scopes, project_id, component
...
func scanToken(row scanner) (apitoken.Token, error) {
	...
	if err := row.Scan(&token.ID, &token.UserID, &token.Name, &token.SecretHash,
		&token.CreatedAt, &expiresAt, &revokedAt, &lastUsedAt, &scopes, &token.ProjectID, &token.Component); err != nil {
