...
func TestAddSystemEnvironmentSupportsGlobalCatalog(t *testing.T) {
	...
	addSystemEnvironment(items, "", "harbor.example.local/noryx-environments/noryx-vscode:0.1.0", systemEnvironmentDefinitions["system-vscode"])

	item, ok := items["|harbor.example.local/noryx-environments/noryx-vscode:0.1.0"]
	if !ok {
