...
func Load() Config {
	...
	if workspaceJupyterImage == "" {
		workspaceJupyterImage = "harbor.example.local/noryx-environments/noryx-python:0.2.1"
	}
	workspaceVSCodeImage := os.Getenv("NORYX_WORKSPACE_VSCODE_IMAGE")
	if workspaceVSCodeImage == "" {
		workspaceVSCodeImage = "harbor.example.local/noryx-environments/noryx-python:0.2.1"
	}
