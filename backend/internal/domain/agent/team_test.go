...
func TestDelegationNeedsBothSidesToHoldTheAction(t *testing.T) {
	...
	if decision := Delegate(lead, holder, ActionRestartApp); !decision.Allowed {
	...
	if decision := Delegate(lead, watcher, ActionRestartApp); decision.Allowed {
	...
	if decision := Delegate(quietLead, holder, ActionRestartApp); decision.Allowed {
...
func TestOnlyALeadDelegates(t *testing.T) {
	...
	if decision := Delegate(operator, other, ActionRestartApp); decision.Allowed {
...
func TestDelegationDoesNotCrossOwnersOrTeams(t *testing.T) {
	...
	if decision := Delegate(lead, stranger, ActionRestartApp); decision.Allowed {
		t.Error("a lead reached an agent belonging to somebody else")
	} else if decision.Refusal != RefusalDifferentOwners {
		t.Errorf("refused for the wrong reason: %s", decision.Refusal)
	}
	if decision := Delegate(lead, elsewhere, ActionRestartApp); decision.Allowed {
	...
	if decision := Delegate(loose, orphan, ActionRestartApp); decision.Allowed {
...
func TestADisabledMemberIsNotAskedToAct(t *testing.T) {
	...
	off.Enabled = false
	if decision := Delegate(lead, off, ActionRestartApp); decision.Allowed {
...
func TestEveryDecisionNamesItsTwoAgents(t *testing.T) {
	...
	other := member("stef", "ops", RoleOperator, []string{ActionRestartApp})
	decision := Delegate(lead, other, ActionRestartApp)
