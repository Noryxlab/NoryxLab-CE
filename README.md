...
- OIDC issuer (current deployment): `https://datalab.example.local/auth/realms/noryx`
...
- DNS policy: wildcard records are not required. Noryx CE routes everything through one hostname (`datalab.example.local`) with path-based routing (`/auth`, `/api`, `/swagger`, `/workspaces`).
...
```bash
BASE_URL=https://datalab.example.local NORYX_USER=stef ./scripts/ops/cleanup-workspaces.sh
```
